...
description: Turn narration, story topics, opinions, or abstract ideas into paper-collage animation. Use source copy, story beats, or a core concept to create visual metaphors, a plan, approved halftone stills, and H3 stop-motion clips, with optional assembly. Keep collage sound effects by default; add music, voiceover, or subtitles only when requested. Use for knowledge explainers, viewpoints, story visuals, and social footage, not presenter ads, editable layers, complex typography, or prompt-only tasks.
...
This Design-adapted Skill uses Design-native image, video, audio, and optional postprocess capabilities. It prioritizes style continuity, color harmony, controlled paper texture, stop-motion collage rhythm, and an audio policy that keeps tactile collage SFX by default while explicitly not adding BGM, voiceover, or subtitles unless the user requests them.
...
- Use the approved still as the visual reference / final-frame anchor whenever the selected Design video model supports it
