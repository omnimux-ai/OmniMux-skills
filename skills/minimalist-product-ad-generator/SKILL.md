...
description: |
  Turn product images and ad requirements into a minimalist product short for e-commerce or launches. Confirm format and variants, extract selling points, write concise English copy, create independent product anchors, plan beat-synced typography, generate a premium H3 product film, and replace its music only when needed. Keep product identity and clean negative space consistent. Use for food, toys, gadgets, and accessories, not talking-head ads, general editing, or complex screen demonstrations.
...
allowed-tools: [question, hub_generate_image, hub_generate_video, hub_generate_music, hub_video_edit, hub_analyse_media, hub_canvas_get_node, hub_save_file_to_session]
---
...
7. **Video model strategy**
   ...
   - If the user-specified model fails, allow at most one targeted retry, then switch to MiniMax-H3 or another available model instead of repeatedly retrying the same model.
