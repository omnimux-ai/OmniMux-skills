...
3. If the user explicitly selected another model and the failure is model-specific, perform one capability re-check and follow the user's choice when supported. Otherwise switch to another available compatible model after explaining why; do not keep retrying the failing model.
