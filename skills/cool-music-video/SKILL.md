...
description: |
  ...
  用户明确提出 MV、音乐视频或 music video，但没有说明时长或完整度时，也使用本 Skill，但只执行入口范围门，询问“完整 MV”还是“15 秒以内单条体验”；未确认短范围前不得读取制作 references 或开始生产。用户已经明确完整 MV、15 秒以上、具体长时长、完整歌曲、多段分镜、音乐分析或终剪合成时不使用，直接进入 MV workflow 或 general-video workflow。
...
- 用户明确完整 MV、15 秒以上、具体长时长、完整歌曲、多段分镜、音乐分析或终剪合成：立即退出本 Skill，转入 MV workflow / general-video workflow。
