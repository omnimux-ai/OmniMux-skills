...
Question 返回的选项、共享补充与后续附加素材分别判断：

- 已选项完成当前确认；同时给出的补充用于解释、约束或修正该决定。只有补充明确推翻已选方向时，才重做候选并重新确认。
- 只给补充文本时，仍按选择、反馈或“你决定 / 直接继续”判断。表达不满意、比较、疑问、修改倾向或不完整想法，先据此改候选，再次用 `question` 确认；无法判断时，用一句话复述准备采用的方向并确认，不直接进入下一阶段。
...
分镜表包含画面、逐字口播和用户能听见的声音，用 `hub_canvas_write_node` 落画布，再用 `question` 确认。用户口述修改时同步更新节点；用户编辑节点后，生成前用 `canvas_get_node` 重读最终版本。修改后重新确认。
