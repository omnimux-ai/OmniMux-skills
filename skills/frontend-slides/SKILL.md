...
description: |
  Create zero-dependency, animation-rich HTML presentations from scratch, rough notes, existing content, or PowerPoint files. Inputs are a topic/notes/PPTX/existing HTML deck; outputs are a distinctive, viewport-safe single-file HTML deck with optional PDF export or shareable URL. Uses visual style previews so users pick aesthetics by seeing, not by describing.
  Use whenever the user wants to build a presentation, convert PPT/PPTX to web, create slides for a talk/pitch, or enhance an existing HTML deck.
---
