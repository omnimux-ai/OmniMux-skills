...
description: H3 video-generation prompt, official H3 prompt format; use when converting a user's natural-language video idea, asset list, keyframe request, multimodal reference set, or edit request into a high-quality H3 prompt through type diagnosis, input/reference fit assessment, targeted clarification, and official-format output.
