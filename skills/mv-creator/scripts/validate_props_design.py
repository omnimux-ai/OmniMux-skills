#!/usr/bin/env python3
"""校验物件设计 design.md 格式是否符合规范。

用法：
  # 校验单个物件
  python validate_props_design.py .mv/{song}/{theme}/props/{prop}/design.md

  # 校验 props 目录下所有物件
  python validate_props_design.py .mv/{song}/{theme}/props/

规则：
  - 必须有一级标题 # 物件名
  - 必须包含 5 个二级标题分区：外观描述、叙事作用、生图设定、候选图、选定图
  - 外观描述必须包含至少 3 项关键词（类型、形态、颜色/材质、尺寸/比例、细节特征）
  - 生图设定必须包含：提示词、模型、参考图
  - 候选图必须有至少 1 条列表项
  - 候选图路径对应的文件必须存在
"""
...
REQUIRED_SECTIONS = ["外观描述", "叙事作用", "生图设定", "候选图", "选定图"]
...
def validate_one(design_path: Path) -> list[str]:
    ...
    # 物件名
    if not data["title"]:
        issues.append(f"ERROR  [{prop_name}] 缺少一级标题 # 物件名")

    # 必需分区
    for section in REQUIRED_SECTIONS:
        if not has_section(data["sections"], section):
            en = SECTION_ALIASES.get(section, "")
            issues.append(f"ERROR  [{prop_name}] 缺少分区 ## {section}/{en}")

    # 外观描述：需要包含至少 3 项列表项
    appearance = get_section(data["sections"], "外观描述")
    if appearance:
        items = [l for l in appearance if LIST_RE.match(l)]
        if len(items) < 3:
            issues.append(
                f"WARN   [{prop_name}] 外观描述/Appearance 列表项不足（需至少 3 项，当前 {len(items)} 项）"
            )

    # 叙事作用：需要有内容
    narrative = get_section(data["sections"], "叙事作用")
    if narrative:
        content = "\n".join(narrative).strip()
        if not content:
            issues.append(f"WARN   [{prop_name}] 叙事作用/Narrative Role 内容为空")
    ...
    if img_settings:
        ...
        for field in PROMPT_FIELDS:
            if not has_prompt_field(content, field):
                issues.append(f"ERROR  [{prop_name}] 生图设定/Image Settings 缺少字段 {field}")
    ...
    if candidates_lines:
        ...
        if not candidates:
            issues.append(f"ERROR  [{prop_name}] 候选图/Candidates 为空（需要至少 1 张）")
        for fname in candidates:
            actual_name = fname.split()[0] if " " in fname else fname
            fpath = prop_dir / actual_name
            if not fpath.exists():
                issues.append(f"WARN   [{prop_name}] 候选图文件不存在: {actual_name}")
    ...
    if selected_lines:
        ...
        if not content:
            issues.append(f"INFO   [{prop_name}] 选定图/Selected 尚未填写（待用户挑选）")
        elif "selected" in content:
            sel_path = prop_dir / "selected.jpg"
            sel_png = prop_dir / "selected.png"
            if not sel_path.exists() and not sel_png.exists():
                issues.append(f"WARN   [{prop_name}] selected.jpg/png 文件不存在")
...
def print_summary(all_issues: dict[str, list[str]]) -> None:
    print("=== 物件设计校验报告 ===\n")
    print(f"物件数量: {len(all_issues)}")
    print()

    total_errors = 0
    total_warns = 0

    for prop_name, issues in all_issues.items():
        ...
        print(f"  [{status}] {prop_name} — {len(errors)} 错误, {len(warns)} 警告")
...
def main():
    if len(sys.argv) < 2:
        print("用法:")
        print("  python validate_props_design.py <design.md 路径>")
        print("  python validate_props_design.py <props 目录路径>")
        sys.exit(1)
    ...
    all_issues = {}
    for dp in designs:
        prop_name = dp.parent.name
        issues = validate_one(dp)
        all_issues[prop_name] = issues
